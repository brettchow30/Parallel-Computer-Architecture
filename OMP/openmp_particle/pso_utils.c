...
/* Return index of best performing particle using OpenMP*/
...
//     #pragma omp parallel for num_threads(num_threads) private(i,particle) shared(g,best_fitness)
//     for (i = 0; i < swarm->num_particles; i++) {
//         particle = &swarm->particle[i];
//         if (particle->fitness < best_fitness) {
//             best_fitness = particle->fitness;
...
{
    int i, j, g, private_g = -1;
    int status;
    float fitness;
    float best_fitness = INFINITY;
    float partial_fitness = INFINITY;
    ...
    #pragma omp parallel private(j,i,particle,fitness,status) firstprivate(partial_fitness,private_g) shared(best_fitness,g) num_threads(num_threads)
    {
         //Parallelize for loop that creates every particle
